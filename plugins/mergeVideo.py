...
chat_id=cb.from_user.id, message_ids=list_message_ids ):
        ...
        try:
            ...
            await cb.message.edit("**🔴 Video is corrupted; Try to add thumbnail first.**")
            return
    ...
    if file_size > 2147483648 and Config.IS_PREMIUM == False:
        await cb.message.edit(
            f"**Video is larger than 2GB and cannot be uploaded.\n\nPurchase premium membership for 4GB support from: {Config.OWNER_USERNAME}.**"
        )
        await delete_all(root=f"downloads/{str(cb.from_user.id)}")
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
        return
    if Config.IS_PREMIUM and file_size > 4294967296:
        await cb.message.edit(
            f"**Video is larger than 4GB and cannot be uploaded due to Telegram limitations.**"
        )
        await delete_all(root=f"downloads/{str(cb.from_user.id)}")
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
